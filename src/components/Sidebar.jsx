...
export default function SideBar({projectTitleList, switchProject, currentProject}) {
    return (
        <div className="flex flex-col w-1/5 min-h-screen bg-[#100e0c] text-white border rounded-tr-2xl gap-y-10 items-center pt-20 overflow-hidden">
            ...
            <Button text="+ Add Project" bgColor=" bg-[#322e2a] " textColor="text-[#9a9692]" handelClick={() => switchProject(-1)}></Button>
            <ul>
                {projectTitleList.map((project) => {
                    {console.log(project.id);}
                    return <ProjectListItem projectName={project.title} isSelected={project.id ==currentProject} id={project.id} switchProject={switchProject}></ProjectListItem>
                })}
